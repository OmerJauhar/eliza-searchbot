    ...
    */
    ...
    constructor(opts: {
        ...
        this.plugins.forEach((plugin) => {
            ...
            plugin.providers?.forEach(async (provider) => {
                if (plugin.name === "plugin-web-search") {
                    let mem: Memory;
                    const output = await provider.get(this, mem);
                    elizaLogger.info(`Plugin Registered: ${output}`);
                }
